...
st.markdown("""
    ...
    .success-box h3, .success-box p, .success-box strong, .success-box em {
...
try:
    ...
    with refresh_col1:
        st.markdown(
            f"<p style='color: #999; font-size: 0.8rem; margin-top: 0.5rem;'>Last updated: {datetime.now().strftime('%I:%M:%S %p')}</p>",
            unsafe_allow_html=True
        )
    with refresh_col2:
